...
class RL_scratch:
    ...
    def get_states_on_db_using_board(self, board, port=5432):
        """
        ...
        try:
            ...
            star = dbt.select_star_with_parameters(sqt.SELECT_STAR_FROM_STATE_WHERE_BOARD, parameters, port)
            return star
        except Exception as e:
            print(e)
            return -1

    def choose_next_position_using_board(self, ttt, port=5432):
        """
        ...
        for index, available_move in enumerate(available_moves):
            ...
            states = self.get_states_on_db_using_board(next_board, port)  # select * from states where board = next_board
            acc = 0
            for state in states:
